...
public class AutoPilot extends SegmentoReta {
     ...
     *         in seconds
     */
    public double time(Vector windSpeed, double linearSpeed) {
        Vector d = this.directionVector().sub(windSpeed);
    ...
    public Vector speed(Vector windSpeed, double time) {
        return (this.directionVector().sub(windSpeed)).mult(1.0 / time);
    }
}
